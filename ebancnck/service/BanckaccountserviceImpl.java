...
return  collect;
    ...
    public void debit(String accountId, double amount, String description) throws BanckAccountNotFoundExeption, BalanceNotSuffisantExeption {
        BanckAccount banckAccount= banckAccountReposetory.findById(accountId).orElseThrow(()->new BanckAccountNotFoundExeption("AcountNotFound"));
         ...
         banckAccountOperation.setAmount(amount);
    ...
    public AccountHistoryDTO getAccountHystory(String idAccount, int page, int size) throws BanckAccountNotFoundExeption {
        ...
        Page<BanckAccountOperation> accountOperations = banckAccountOperationReposetory.findByBanckAccountIdOrderByOperationDateDesc(idAccount, PageRequest.of(page, size));
    ...
    @Override
    public ClientAccountDTO getCustomerAccounts(Long clientId, int page, int size) {
        Page<BanckAccount> customerAccounts =banckAccountReposetory.findBanckAccountByClient_Id(clientId, PageRequest.of(page, size));

        ClientAccountDTO
                clientAccountDTO = new ClientAccountDTO();
        clientAccountDTO.setClientId( clientId );
        clientAccountDTO.setCurrentPage( page);
        clientAccountDTO.setTotalPages( customerAccounts.getTotalPages() );
        clientAccountDTO.setPageSize(size);
        clientAccountDTO.setAccounts(
                customerAccounts.getContent().stream().map( bankAccount -> {
                    if (bankAccount instanceof SavinAccount)
                        return banckAccountMappers.fromSavinAccount((SavinAccount) bankAccount);
                    return banckAccountMappers.fromCurrentAccount((CurrentAccount) bankAccount);
                }).collect(Collectors.toList())
        );
